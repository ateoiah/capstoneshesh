...
if (isset($_POST['addrestaurantbtn'])) {
    ...
    if (mysqli_num_rows($result_email) === 0) {
        // Email is not registered in admintb
        $_SESSION['error'] = "The email is not registered as an owner. Register an email first.";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $query = "INSERT INTO restauranttb (restaurant_name, address, contactnumber, restaurant_email) VALUES (?, ?, ?, ?)";
        $stmt = mysqli_prepare($connection, $query);
        mysqli_stmt_bind_param($stmt, 'ssss', $restaurantname, $adrress, $contactnumber, $email);
